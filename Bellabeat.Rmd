...
```{r}
daily_activity <- read.csv("dailyActivity_mergedApril.csv", header = TRUE)
daily_steps <- read.csv("dailySteps_merged.csv", header  = TRUE)
daily_sleep <- read.csv("sleepDay_merged.csv", header = TRUE)
...
weightLogInfo_merged <- read.csv("weightLogInfo_merged.csv", header = TRUE)
met_narrow <- read.csv("minuteMETsNarrow_merged.csv", header = TRUE)
daily_activity_sleep_met <- read.csv("daily_activity_sleep_met.csv", header = TRUE)
totalactivity <- read.csv("totalactivity.csv", header = TRUE)
```
...
```
And then I need to change the date format to be in a more conventional form.

```{r}
daily_activity_sleep_mets <- daily_activity_sleep_mets %>%
...
I wanted to know what days people were achieving the recommended number of steps.

```{r}
daily_steps <- daily_steps %>%
  group_by(day) %>%
  summarize(daily_steps = mean(steptotal))
```

```{r}
ggplot(daily_steps, aes(day, daily_steps)) +
  geom_col(fill = "pink") +
  geom_hline(yintercept = 7000) +
  labs(title = "Steps per Day", x="",y="") +
...
```{r}
ggplot(daily_sleep, aes(day, daily_sleep)) +
  geom_col(fill = "red") +
